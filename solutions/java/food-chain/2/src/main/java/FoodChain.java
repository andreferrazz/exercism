class FoodChain {
    ...
    String verse(int verseNumber) {
        ...
        StringBuilder result = new StringBuilder("I know an old lady who swallowed a ")
                .append(animals[verseNumber - 1])
                .append(".\n");

        if (verseNumber > 1) {
            result.append(phrases[verseNumber - 2]).append("\n");
            result.append(getSentences(verseNumber - 1));
        }
    ...
    StringBuilder getSentences(int verseNumber) {
        StringBuilder sentences = new StringBuilder();
        for (int i = verseNumber; i > 0; i--) {
            if (animals[i].equals("bird")) {
                sentences.append("She swallowed the bird to catch the spider that wriggled and jiggled and tickled inside her.\n");
            } else {
                sentences.append("She swallowed the ")
                        .append(animals[i])
                        .append(" to catch the ")
                        .append(animals[i - 1])
                        .append(".\n");
